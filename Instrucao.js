...
function Instrucao(canvas){
	...
	//Trilha Sonora
	trilha_instrucao = new Audio();
	trilha_instrucao.src = "musicas/Credits theme.mp3";
	trilha_instrucao.volume = 0.5;
	...
	this.returnState = function(){
		if(gameState == thisScene){
			...
			trilha_instrucao.load();
			trilha_instrucao.pause();
			return nextScene;
		}
	}
	...
	this.loop = function (){
		if(gameState == thisScene)
		{
			...
			this.update();
			this.draw();
			trilha_instrucao.play();
		}
		else{
			this.canvasFade("out");
			console.log("fim das Instrucoes!");
		}
	}
