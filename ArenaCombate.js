...
function ArenaCombate(canvas, player){
	...
	//Intro magus
	var magus_intro = new Objetos(null);
	magus_intro.setSprite("img/layouts/falas/magus_msg_0.png");
	magus_intro.transform(115, 0, 500, 100); 					// objeto.tamanho do objeto(Posição X, Posição Y, Largura, Altura);
	magus_intro.showImg = 0;
	var introduction = 0;
	var countIntro = 0;
	var deltaIntro = 0.05;
	...
	//Skills animation
	var heal = new Objetos(null);
	heal.transform(254, 75, 220, 20); 			// objeto.tamanho do objeto(Posição X, Posição Y, Largura, Altura);
	heal.setBattleAnimationDetails("img/layouts/skills/heal.png", 280, 230, 192, 192, [5], ["heal"]);//(sprite, posX, posY, width, height, rangeVector, actionsVector)
	heal.enableBattleMode();
	heal.countAnim = 999;
	
	
	var lightning = new Objetos(null);
	lightning.transform(254, 75, 220, 20); 			// objeto.tamanho do objeto(Posição X, Posição Y, Largura, Altura);
	lightning.setBattleAnimationDetails("img/layouts/skills/lightning.png", 300, -165, 128, 512, [5], ["lightning"]);//(sprite, posX, posY, width, height, rangeVector, actionsVector)
	lightning.enableBattleMode();
	lightning.countAnim = 999;
	...
	this.setParametros = function(background, track1, intro, _enemies, joia){
	...
	function mouseClicked(e){
		if(gameState == thisScene){
			...
			if(player.runAnimation == false && inimigo.runAnimation == false && heal.runAnimation == false && lightning.runAnimation == false && magus_intro.showImg == 0){
				...
				if(usarItem == 1){
					...
					if(buttonLeftMana <= mouseX && mouseX <= buttonRightMana && buttonTopMana <= mouseY && mouseY <= buttonDownMana){
						...
						if(player.inventory.items["potionMp"] <= 0){
							//alert("Nao ha mana-potions.");
							console.log("Nao ha mana-potions.");
						}
						else{
							usarPotionMana = 1;
							barraItens.showImg = 0;
	...
	this.update = function(){
		...
		if(introduction == 1){
			magus_intro.showImg = 1;
			player.runAnimation = false;
			inimigo.runAnimation = false;
		}
		
		//Turno do PLAYER - Aguarda fim da animação do inimigo
		if(inimigo.runAnimation == false && heal.runAnimation == false && introduction == 0){
			...
			if(playerWait >= 10){
				...
				if(turno == 1 && (atacar == 1 || usarHabilidade == 1 || usarPotion == 1 || usarPotionMana == 1) && player.atributos.lifePoints > 0){
					...
					if(usarHabilidade == 1){
						...
						lightning.selectAction("lightning");
		...
		if(player.runAnimation == false && lightning.runAnimation == false && introduction == 0){	
		...
		if(countIntro <= 80 && magus_intro.showImg == 1){
			countIntro+= deltaIntro;
			
			if(countIntro >= 25 && countIntro <= 50){
				magus_intro.setSprite("img/layouts/falas/magus_msg_1.png");
			}
			if(countIntro > 50 && countIntro <= 65){
				magus_intro.setSprite("img/layouts/falas/magus_msg_2.png");
			}
			if(countIntro > 65 && countIntro <= 73){
				magus_intro.setSprite("img/layouts/falas/magus_msg_3.png");
			}
			...
			if(countIntro >= 73){
				...
				player.selectAction("intro");
				//player.srcX = 0;
				//player.countAnim = 0;
				inimigo.runAnimation = true;
				inimigo.selectAction("intro");
				//inimigo.srcX = 0;
				//inimigo.countAnim = 0;
